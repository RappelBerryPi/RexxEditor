...
import org.imperfectmommy.rexxeditor.scanner.tokenRules.ClassDefinitionRule;
import org.imperfectmommy.rexxeditor.scanner.tokenRules.IRexxTokenRule;
import org.imperfectmommy.rexxeditor.scanner.tokenRules.KeywordRule;
import org.imperfectmommy.rexxeditor.scanner.tokenRules.MethodRule;
import org.imperfectmommy.rexxeditor.scanner.tokenRules.VariableDefinitionRule;
import org.imperfectmommy.rexxeditor.scanner.tokenRules.VariableRule;
...
public class RexxLine {
    ...
    public int getPrevTokenPos(int position, boolean whitespace) {
        ...
        if (whitespace == true) {
            while (tempToken.isWhitespace() || tempToken.equals(tokenList.rexxComment)) {
                position--;
                if (position >= 0) tempToken = this.get(position).getToken();
                else break;
    ...
    public int getFirstTokenPos() {
        ...
        while (tempToken.isWhitespace() || tempToken.equals(tokenList.rexxComment)) {
            i++;
            if (i < fPosition) tempToken = this.get(i).getToken();
            else break;
    ...
    public String toString() {
        ...
        for (int i = 0; i < fTokenList.size(); i++) {
            RexxToken token = fTokenList.get(i);
            result += "index" + i;
